...
import com.Pages.Settings_KDS_Configuration_Page;
...
public SelfHealingDriver driver;
	...
	ExtentTest test = rep.startTest("KDS Configuration");
	...
	Settings_KDS_Configuration_Page kds;
...
//		driver = new ChromeDriver(chromeOptions);
	...
	{
		Thread.sleep(10000);
		Open_KDS_Page(driver);
		VerifyTheField_Options(driver);
		VerifyTheStyleOptions(driver);
		VerifyThe_SMS_Order_Notifications(driver);
	}
	
	@Test(priority = 3,enabled = false)
	public void Open_KDS_Page(SelfHealingDriver driver) throws Exception
	{
		...
		driver.get(Utility.getProperty("baseURL")+Utility.getProperty("store_Id2")+"kds");

		driver.manage().timeouts().implicitlyWait(Duration.ofSeconds(30));
		Thread.sleep(15000);
		
		kds.verifyTheKDSPage();
		//cmp.VerifyMainScreenPageHeader("Customer Display System (CDS)");
	}
	
	@Test(priority = 3,enabled = false)
	public void VerifyTheField_Options(SelfHealingDriver driver) throws Exception
	{
		kds=new Settings_KDS_Configuration_Page(driver, test);
		
		Thread.sleep(1000);
		kds.verifyTheFieldOptions();
	...
	@Test(priority = 3,enabled = false)
	public void VerifyTheStyleOptions(SelfHealingDriver driver) throws Exception
	{
		kds=new Settings_KDS_Configuration_Page(driver, test);
		
		Thread.sleep(1000);
		kds.verifyTheStyleOptions();
	...
	@Test(priority = 3,enabled = false)
	public void VerifyThe_SMS_Order_Notifications(SelfHealingDriver driver) throws Exception
	{
		kds=new Settings_KDS_Configuration_Page(driver, test);
		
		Thread.sleep(1000);
		kds.verifyTheSMSOrderNotifications();
