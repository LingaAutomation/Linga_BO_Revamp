...
public class Old_BO_Reports_Sales_CashierOut_Report 
{
public SelfHealingDriver driver;
	
	
	ExtentReports rep = ExtentManager.getInstance();
	ExtentTest test = rep.startTest("Old BO - Sales Report - Cashier Out");
...
//		driver=new ChromeDriver(chrOpt);
	...
	public void Open_SaleRecap_Sales_Page(SelfHealingDriver driver) throws Exception
	{
		...
		driver.get(Utility.getProperty("baseURL_Old_BO")+Utility.getProperty("store_Id")+"report/cashierOut");

		Thread.sleep(5000);
		//Check whether the dashboard page refreshed or not
		if(driver.findElement(By.xpath("//li[contains(.,'Cashier out')][@ui-sref-active='active']")).isDisplayed())
		{
			test.log(LogStatus.PASS, "Cashier Out Report page opened");
		}
		else
		{
			test.log(LogStatus.FAIL, "Cashier Out Report page not open");
		}
		
	}
	
	@Test(priority = 4,enabled = false)
	public void Old_BO_SaleRecap_Report_Date_Range(SelfHealingDriver driver) throws Exception
	{
		repts=new ReportsPage(driver, test);
		cmp=new Common_XPaths(driver, test);
		ExcelDataConfig excel=new ExcelDataConfig(Utility.getProperty("BO_Comparision"));
		
		//Click the Cashier Out Type
//		driver.findElement(By.xpath("//label[contains(.,'Cashier Out Type')]/../div/a")).click();
		...
		{
			test.log(LogStatus.INFO, "Sale Report for Cashier Out Not Available for Date Range");
		
			excel.setreportData("Sales_Report", 46, 3, st);
			excel.setreportData("Sales_Report", 47, 3, st);
			excel.setreportData("Sales_Report", 48, 3, st);
			excel.setreportData("Sales_Report", 49, 3, st);
			excel.setreportData("Sales_Report", 50, 3, st);
			excel.setreportData("Sales_Report", 51, 3, st);
			excel.setreportData("Sales_Report", 52, 3, st);
			excel.setreportData("Sales_Report", 53, 3, st);
			excel.setreportData("Sales_Report", 54, 3, st);
			excel.setreportData("Sales_Report", 55, 3, st);
			excel.setreportData("Sales_Report", 56, 3, st);
			excel.setreportData("Sales_Report", 57, 3, st);
		...
		catch(Exception G)
		{
			...
			test.log(LogStatus.PASS, "Sale Report for Cashier Out Available for Date Range");
...
//					double ActualDiscount=Double.parseDouble(Discnt);
					...
					excel.setreportData("Sales_Report", 55, 3, Refund);
				}
			}
			catch(Exception k)
			{
...
//			double ActualDiscount=Double.parseDouble(Discnt);
			...
			excel.setreportData("Sales_Report", 46, 3, NetSales);
			excel.setreportData("Sales_Report", 47, 3, Tax);
			excel.setreportData("Sales_Report", 48, 3, Discount);
			excel.setreportData("Sales_Report", 49, 3, GrandSales);
			excel.setreportData("Sales_Report", 50, 3, GrossReceipt);
			excel.setreportData("Sales_Report", 51, 3, GrossSales);
			excel.setreportData("Sales_Report", 52, 3, TaxExempt);
			excel.setreportData("Sales_Report", 53, 3, NetVoid);
			excel.setreportData("Sales_Report", 54, 3, GrossVoid);
			excel.setreportData("Sales_Report", 56, 3, TaxRoundoff);
			excel.setreportData("Sales_Report", 57, 3, DiscountTax);
