...
//		Add_Central_Warehouse_with_Copy_From_Store(driver);
//		Create_Duplicate_Central_Warehouse(driver);
//		Central_Warehouse_Report(driver);
	}
	
	@Test(priority = 3,enabled = false)
	public void Open_Central_Warehouse_Page(SelfHealingDriver driver) throws Exception
	{
...
//		a.Navigate_To_Page(Utility.getProperty("store_Id1"), "Central Warehouses");
	...
	public void Add_Central_Warehouse(SelfHealingDriver driver) throws Exception
	{
		civ=new Enterprise_CentralIventoryPage(driver, test);
		cmp=new Common_XPaths(driver, test);
		...
		//Enter the Email ID
		new Settings_StoreInformation_Page(driver, test).Enter_Email_Input(Utility.getProperty("Central_emailId"));
	...
	public void Edit_and_Close_Cancel_Central_Warehouse(SelfHealingDriver driver) throws Exception
	{
		cmp=new Common_XPaths(driver, test);
		civ=new Enterprise_CentralIventoryPage(driver, test);
	...
	public void Edit_and_Update_Central_Warehouse(SelfHealingDriver driver) throws Exception
	{
		cmp=new Common_XPaths(driver, test);
		civ=new Enterprise_CentralIventoryPage(driver, test);
...
//	
	...
	@Test(priority = 7,enabled = false)
	public void Add_Central_Warehouse_with_Copy_From_Store(SelfHealingDriver driver) throws Exception
	{
		cmp=new Common_XPaths(driver, test);
		civ=new Enterprise_CentralIventoryPage(driver, test);
		...
		Thread.sleep(500);
		//Enter the Central Warehouse Name
		cmp.EnterName(Utility.getProperty("New_Central_WareHouse")+"a");
		
		//Enter the Description
		new DepartmentPage(driver, test).Enter_Description("This Central Inventory-Central Warehouse (Copy from existing Store) is : "+Utility.getProperty("New_Central_Warehouse")+ "For Warehouse Use");
		...
		if(cmp.ConfirmationAlertMsg().getText().equalsIgnoreCase("Central Warehouse Saved Successfully"))
		{
			test.log(LogStatus.PASS, "Central Warehouse Saved Successfully Enabled with Copy from Existing Store");
		
			ut.PassedCaptureScreenshotAsBASE64(driver, test);
		}
		else
		{
			test.log(LogStatus.FAIL, "Central Warehouse Saved Failed Enabled with Copy from Existing Store");
	...
	public void Create_Duplicate_Central_Warehouse(SelfHealingDriver driver) throws Exception
	{
		civ=new Enterprise_CentralIventoryPage(driver, test);
		cmp=new Common_XPaths(driver, test);
	...
	@Test(priority = 9,enabled = false)
	public void Central_Warehouse_Report(SelfHealingDriver driver) throws Exception
	{
		civ=new Enterprise_CentralIventoryPage(driver, test);
		cmp=new Common_XPaths(driver, test);
