...
public class Product_Checks_AddEditDelete_Open_Check_Discount {
...
//					ut.FailedCaptureScreenshotAsBASE64(driver, test);
	...
	public void Edit_and_Close_Cancel_Open_Check_Discount() throws Exception
	{
		...
		Thread.sleep(1000);
		try {
			if(driver.findElement(By.xpath("//button[contains(.,'OPEN CHECK') and contains(@class,'active')]")).isDisplayed()) {
				test.log(LogStatus.PASS, "Open Check Discount Screen is displayed when user click the back button from edit screen");
			}
		}catch(Exception s) {
			test.log(LogStatus.FAIL, "Open Check Discount Screen is not displayed when user click the back button from edit screen");
	...
	Thread.sleep(3000);
	//Enter the Name
	cmp.EnterName(Utility.getProperty("DiscountName_OpenCheckAMT")+"a");
...
//					test.log(LogStatus.PASS, "Coupon Code");
			...
			if(cmp.Save_and_PublishButton().isEnabled())
			{
				...
				{
					test.log(LogStatus.FAIL, "Check Based Discount Saved with Entering Special Characters");

					ut.FailedCaptureScreenshotAsBASE64(driver, test);

					driver.findElement(By.xpath("//button[contains(.,'EDIT')]")).click();
					//cmp.SearchAndClickEdit(Utility.getProperty("DiscountName_CheckBased_AutoDiscount"));

				}
				else
				{
					test.log(LogStatus.FAIL, "Check Based Discount Saved Failed with Entering Special Characters");
				}
	...
	//Enter the Safety Limit Percentage
		...
		try {
			if(driver.findElement(By.xpath("//button[contains(.,'OPEN CHECK') and contains(@class,'active')]")).isDisplayed()) {
				test.log(LogStatus.PASS, "Open Check Discount Screen is displayed when user edit the open check discount");
			}
		}catch(Exception s) {
			test.log(LogStatus.FAIL, "Open Check Discount Screen is not displayed when user edit the open check discount");
	...
	Thread.sleep(500);
	//Enter the Name
	cmp.EnterName(Utility.getProperty("DiscountName_OpenCheckAMT")+"b");
	...
	Thread.sleep(3000);
	//Enter the Name
	cmp.EnterName(Utility.getProperty("DiscountName_OpenCheckAMT")+"c");
	...
	dcp.Enter_Safety_Limit_Percentage("20000");
		...
		if(cmp.ConfirmationAlertMsg().getText().equalsIgnoreCase("Updated Successfully"))
		{
			test.log(LogStatus.PASS, "Open Check Discount updated successfully to Menu Check");
	
			ut.PassedCaptureScreenshotAsBASE64(driver, test);
		}
		else
		{
			test.log(LogStatus.FAIL, "Open Check Discount updated fail to Menu Check");
	...
	//Search and Click Delete button
		...
		try {
			if(driver.findElement(By.xpath("//button[contains(.,'DEACTIVATE')]")).isDisplayed()) {
				...
				{
					test.log(LogStatus.FAIL, "Check Based Deleted when clicking Cancel button");
				...
				{
					test.log(LogStatus.PASS, "Check Based not Deleted when Clicking Cancel button");
